...
typedef struct new_movie{
	...
	struct new_movie *left;	/* Pointer to the node's left child*/
	struct new_movie *right;	/* Pointer to the node's right child*/
}new_movie_t;
...
typedef struct movie{
	...
	int watchedCounter;	 		/* How many users rate the movie*/
	int sumScore;				/* The sum of the ratings of the movie*/
	struct movie *left;	/* Pointer to the node's left child*/
	struct movie *right;	/* Pointer to the node's right child*/
}movie_t;
...
typedef struct user_movie{
	...
	struct user_movie *left;	/* Pointer to the node's left child*/
	struct user_movie *right;	/* Pointer to the node's right child*/
}userMovie_t;
...
/*
 * A singly linked list. Acts as a helping structure to construct the
...
extern const int primes_g[170];



extern list_node* cat_table[6];
extern bool is_first_time;
extern int number_of_nodes;

class hashtable;

extern hashtable* user_hashtable_p;
...
int hashing_function(int key, int length);
